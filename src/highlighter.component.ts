...
export class NgHighlighterComponent implements OnChanges, AfterContentInit {
  ...
  private highlight(line: string) {
    ...
    this.patterns.forEach(
      (pattern: NgHighlighterPatternDirective) => {
    ...
    [...tags]
      ...
      .forEach((tag: Tag) => {
        ...
        if (contents.length === expectedLength) {
          ...
          parts.push(
            `<span class="highlighted ${tag.type || ''}" rel="${tag.type}">${tag.formatter(contents)}</span>`
          );
          prevTags.push(tag);
        }
      });
  ...
  private getMatchedElement(event: MouseEvent): Highlighted {
    let matched = this.highlightedElements.find(
      (el: Highlighted): boolean => isCoordinateWithinRect(
        el.clientRect,
        event.clientX,
        event.clientY
      )
