...
describe('APIDocVisitor', () => {
  ...
  it('should extract basic type info from classes', () => {
    ...
    expect(docs.NthDirective.type).toBe('Directive');
    expect(docs.NthComponent.type).toBe('Component');
    expect(docs.NthService.type).toBe('Service');
    expect(docs.NthClass.type).toBe('Class');
    expect(docs.NthInterface.type).toBe('Interface');
  });
  ...
  it('should extract documentation from services', () => {
    const serviceDocs = apiDoc(['./misc/api-doc-test-cases/services-with-methods.ts']).MyService;

    expect(serviceDocs.fileName).toBe('./misc/api-doc-test-cases/services-with-methods.ts');
    expect(serviceDocs.className).toBe('MyService');
  ...
  it('should extract documentation of properties from services', () => {
    const serviceDocs = apiDoc(['./misc/api-doc-test-cases/services-with-properties.ts']).MyConfig;
    ...
    expect(serviceDocs.properties[1].description).toBe('<p>Maximal value to be displayed</p>');
  ...
  it('should extract documentation from interfaces', () => {
    const interfaceDocs = apiDoc(['./misc/api-doc-test-cases/interface-with-properties.ts']).NthOptions;

    expect(interfaceDocs.className).toBe('NthOptions');
