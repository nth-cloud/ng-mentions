# Building and Testing ngx-mentions

This document describes how to set up your development environment to build and test ngx-mentions.
...
Before you can build and test ngx-mentions, you must install and configure the
...
Fork and clone the ngx-mentions repository:
...
git clone git@github.com:<github username>/ngx-mentions.git ngx-mentions

# Go to the ngx-mentions directory:
cd ngx-mentions
...
Next, install the JavaScript modules needed to build and test ngx-mentions:

```shell
# Install ngx-mentions project dependencies (package.json)
npm install
```
...
* You can run this as a **git pre-commit hook** to automatically format your delta regions when you
  commit a change. In the ngx-mentions repo, run
